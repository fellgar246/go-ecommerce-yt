...
import (
	...
	"github.com/fellgar246/go-ecommerce-yt/models"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if count > 0 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "this phone no. is already registered"})
			return
		}

		password := MashPasword(*user.Password)
		user.Password = &password

		user.Created_At, _ = time.Parse(time.RFC339, time.Now().Format(time.RFC3339))
		user.Updated_At, _ = time.Parse(time.RFC339, time.Now().Format(time.RFC3339))
